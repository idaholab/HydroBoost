...
def create_mean_persistence_forecast(
    ...
    The mean persistence logic:
    - For each forecast date, compute the mean of the first 24 hours from the 
      previous num_mean_persistence_days forecasts
    - Tile this mean pattern to fill remaining forecast hours (or all hours if not using 
    ...
    def get_forecast(df_tpl: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in range(len(df_result.index)):
            ...
            if use_perfect_foresight_first_day:
                ...
                # Tile means to cover remaining days after first day
...
if __name__ == "__main__":
    """
    Command-line interface for testing mean persistence models.
    
    Usage:
        python mean_persistence_models.py Model_A
        python mean_persistence_models.py Model_B
        python mean_persistence_models.py Model_C
    ...
    # Calculate differences for remaining hours (where persistence is used)
