...
function build_HydroBoost_optimization_model_instance!(am::Abstract_ALEAF_Model, day_id, prior_day_solution)
    ...
    # (30) BESS energy throughput over the look-ahead window
    ...
    ### Hydro power system constraints: from (30) to (50) ###
    for h in ids_h
        for t in ids_t
    ...
    ### RES generators constraints: from (51) to (53) ###
    for h in ids_h
        for t in ids_t
            for k in ids_k_ren
    ...
    ### Coupling constraints: from (54) to (58)###
    for h in ids_h
        for t in ids_t
...
### Hydroelectric System without Reservoir == Run-of-River System ###
...
# Constraints (47) and (48) already included in section "Define decision variables".
...
# (o) Startup and shutdown daily limit of hydro generators
...
# (p) Non-dispatchable renewable generation constraints:
...
# Constraint (53) already included in section "Define decision variables".

### Coupling constraints ###

# (q) Hydro + BESS + RES power balances and interconnection limits:
...
function HydroBoost_constraint_outflow_interconnection_limit_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, h::Int, t::Int; const_name_flag::Bool=false)
    ...
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        (p_B_DT_ht + p_G_Gr_ht + p_R_Gr_ht + r_RU_ht + r_SR_ht) - outflow_limits
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr <= 0
        )    
...
function HydroBoost_constraint_inflow_interconnection_limit_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, h::Int, t::Int; const_name_flag::Bool=false)
    ...
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        p_Gr_St_ht + r_RD_ht- inflow_limits
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr <= 0
        )    
...
function HydroBoost_report_result_plant_dispatch(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data)
    ...
    "p_G_ht", "p_GB_ht", "p_G_Gr_ht", "q_G_jht", "e_H_ht", "I_ht", "s_ht",
...
function HydroBoost_report_result_hydro_dispatch(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data)
    ...
    row = 2
    for day_id in repday_ids, h in ids_h, t in ids_t, unit_idx in eachindex(ids_j_hydro)
...
function HydroBoost_report_result_storage_dispatch(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data)
    ...
    # Fill rows with solution data
    row = 2
    for day_id in repday_ids, h in ids_h, t in ids_t, unit_idx in eachindex(ids_i_sto)
...
function HydroBoost_report_result_objective(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data)
    ...
    for day_id in repday_ids
        ...
        for h in ids_h, t in ids_t
            ...
            # Objective at (h,t)
            obj_ht = energy_arbitrage + as_bess + reg_adj_bess + startup_shutdown_cost + rough_zone_penalty + vom_cost                          
...
function generate_networkdata_HydroBoost(ALEAF_setting::Dict{String,<:Any}, project_id, case_id; year_id::Int64=1)
    ...
    allocate_hydro_sub_hourly_timeseries_data!(timeSeries_df, network_data, ALEAF_setting, ["Inflow", "Water_Release_Requirement", "Diversion"])
