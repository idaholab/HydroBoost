...
function build_HydroBoost_optimization_model_instance!(am::Abstract_ALEAF_Model, day_id, prior_day_solution)
    ...
    ### Coupling constraints: from (74) to (81) ###
    for h in ids_h
        for t in ids_t
		    for p in ids_p
				...
				HydroBoost_constraint_Hydro_power_generation_coupling_pht(JuMP_model, am, "HydroBoost_constraint_Hydro_power_generation_coupling_pht", p, h, t; const_name_flag)
				HydroBoost_constraint_RES_power_generation_coupling_pht(JuMP_model, am, "HydroBoost_constraint_RES_power_generation_coupling_pht", p,h, t; const_name_flag)
				HydroBoost_constraint_ES_power_generation_coupling_pht(JuMP_model, am, "HydroBoost_constraint_ES_power_generation_coupling_pht", p, h, t; const_name_flag)
			end
        end
    end
    ...
    for h in ids_h
        for t in ids_t
		    for p in ids_p
				HydroBoost_constraint_outflow_interconnection_limit_pht(JuMP_model, am, "HydroBoost_constraint_outflow_interconnection_limit_pht", p, h, t; const_name_flag)
				HydroBoost_constraint_inflow_interconnection_limit_pht(JuMP_model, am, "HydroBoost_constraint_inflow_interconnection_limit_pht", p, h, t; const_name_flag)
			end
        end
    end
...
function HydroBoost_objective_function(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, ids_p, ids_j_hydro, ids_i_sto, ids_h, ids_t, ids_l)
    ...
    # Energy revenue (plant-level, consistent with base Model_B formulation)
    for h in ids_h
        for t in ids_t
            for p in ids_p
                JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["DA_LMP"], get_variable(am, :p_G_Gr_pht, (p,h,t)))
                JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["DA_LMP"], get_variable(am, :p_B_DT_pht, (p,h,t)))
                JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["DA_LMP"], get_variable(am, :p_R_Gr_pht, (p,h,t)))
                JuMP.add_to_expression!(objective, -1.00001 * am.ref[:nw][0][:repdays]["data"][h][t]["DA_LMP"], get_variable(am, :p_Gr_St_pht, (p,h,t)))
            end
        end
    end
...
# Constraint (77) --- MOD
function HydroBoost_constraint_Hydro_power_generation_coupling_pht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, p::Int64, h::Int, t::Int; const_name_flag::Bool=false)
    
    # parameter  
    
    # variable
    p_G_pht = get_variable(am, :p_G_pht, (p,h,t))
    p_G_Gr_pht = get_variable(am, :p_G_Gr_pht, (p,h,t))
    p_GB_pht = get_variable(am, :p_GB_pht, (p,h,t))
        
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        p_G_pht - (p_G_Gr_pht + p_GB_pht)
        )# plant level   
    constraint = JuMP.@constraint(JuMP_model, 
        expr == 0
        )    
...
# Constraint (78) --- MOD
function HydroBoost_constraint_RES_power_generation_coupling_pht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, p::Int64, h::Int, t::Int; const_name_flag::Bool=false)
    
    # parameter  
    
    # variable
    p_R_pht = get_variable(am, :p_R_pht, (p,h,t))
    p_R_Gr_pht = get_variable(am, :p_R_Gr_pht, (p,h,t))
    p_R_St_pht = get_variable(am, :p_R_St_pht, (p,h,t))
        
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        p_R_pht - (p_R_Gr_pht + p_R_St_pht)
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr == 0
        )    
...
# Constraint (79) --- MOD
function HydroBoost_constraint_ES_power_generation_coupling_pht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, p::Int64, h::Int, t::Int; const_name_flag::Bool=false)#revised, DONE
    ...
    p_GB_pht = get_variable(am, :p_GB_pht, (p,h,t))
    p_R_St_pht = get_variable(am, :p_R_St_pht, (p,h,t))
    p_Gr_St_pht = get_variable(am, :p_Gr_St_pht, (p,h,t))
            
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        p_B_CT_pht - (p_GB_pht + p_R_St_pht + p_Gr_St_pht)
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr == 0
        )    
...
# Constraint (80) --- MOD
function HydroBoost_constraint_outflow_interconnection_limit_pht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, p::Int64, h::Int, t::Int; const_name_flag::Bool=false)
    ...
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        (p_B_DT_pht + p_G_Gr_pht + p_R_Gr_pht + r_RU_GB_pht + r_SR_GB_pht) - outflow_limits
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr <= 0
        )    
...
# Constraint (81) --- MOD
function HydroBoost_constraint_inflow_interconnection_limit_pht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, p::Int64, h::Int, t::Int; const_name_flag::Bool=false)
    ...
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        p_Gr_St_pht + r_RD_GB_pht - inflow_limits
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr <= 0
        )    
...
function HydroBoost_report_result_plant_dispatch(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data) # --- MOD
    ...
    "p_G_pht", "p_GB_pht", "p_G_Gr_pht", "r_RU_G_pht", "r_RD_G_pht", "r_SR_G_pht", "q_G_jht", "e_H_pht", "Inflow", "s_pht",
    "p_B_DT_pht", "p_B_CT_pht", "p_R_pht", "p_R_Gr_pht", "p_R_St_pht", "p_Gr_St_pht", "r_RU_pht", "r_RD_pht", "r_SR_pht",
        ...
        for hour_id in ids_h
            for time_id in ids_t
                ...
                for plant_i in eachindex(ids_p)
                    ...
                    ### REMOVED ### e_B_pht = daily_solutions[string(day_id)]["solution"]["storage"][idx_pht]["e_B_pht"]
					p_Gr_St_pht = daily_solutions[string(day_id)]["solution"]["storage"][idx_pht]["p_Gr_St_pht"]
					...
					p_GB_pht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_pht]["p_GB_pht"]
					p_G_Gr_pht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_pht]["p_G_Gr_pht"]
					...
					p_R_Gr_pht = daily_solutions[string(day_id)]["solution"]["renewable"][idx_pht]["p_R_Gr_pht"]
					p_R_St_pht = daily_solutions[string(day_id)]["solution"]["renewable"][idx_pht]["p_R_St_pht"]
					...
					dispatch_output_list[row_id, :] = [day_id, hour_id, time_id, p, LMP, RT_LMP,Reg_up_price, Reg_dn_price, Spin_price, Delta_RU, Delta_RD,
						p_G_pht, p_GB_pht, p_G_Gr_pht, r_RU_G_pht, r_RD_G_pht, r_SR_G_pht, q_G_jht, e_H_pht, Water_Inflow, s_pht,
                        p_B_DT_pht, p_B_CT_pht, p_R_pht, p_R_Gr_pht, p_R_St_pht, p_Gr_St_pht, r_RU_pht, r_RD_pht, r_SR_pht,
                        r_RU_GB_pht, r_RD_GB_pht, r_SR_GB_pht]
				end	
            end
...
function HydroBoost_report_result_objective(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data) # --- MOD
    ...
    for day_id in repday_ids
        ...
        for h in ids_h, t in ids_t
            ...
            # Energy arbitrage term (plant-level, aligned with objective function)
            energy_arbitrage = 0.0
            ...
            for p in ids_p
				...
				p_G_Gr_pht  = daily_solutions[day_key]["solution"]["hydro"][idx_pht]["p_G_Gr_pht"]
				p_B_DT_pht  = daily_solutions[day_key]["solution"]["storage"][idx_pht]["p_B_DT_pht"]
				p_R_Gr_pht  = daily_solutions[day_key]["solution"]["renewable"][idx_pht]["p_R_Gr_pht"]
				p_Gr_St_pht = daily_solutions[day_key]["solution"]["storage"][idx_pht]["p_Gr_St_pht"]
                energy_arbitrage += LMP * (p_G_Gr_pht + p_B_DT_pht + p_R_Gr_pht - 1.00001 * p_Gr_St_pht)
                energy_import_cost += LMP * p_Gr_St_pht
            end
