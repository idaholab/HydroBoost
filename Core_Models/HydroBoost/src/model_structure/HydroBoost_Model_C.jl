...
HydroBoost Model C: Pumped Storage Hydro (PSH) Plant
...
function build_HydroBoost_optimization_model_instance!(am::Abstract_ALEAF_Model, day_id, prior_day_solution)
    ...
    HydroBoost_variable_ht_real(JuMP_model, am, :storage, "r_RU_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                  # Reserve for regulation up provided by all BESS units in hour t [MWh]
    HydroBoost_variable_ht_real(JuMP_model, am, :storage, "r_RD_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                  # Reserve for regulation down provided by all BESS units in hour t [MWh]
    HydroBoost_variable_ht_real(JuMP_model, am, :storage, "r_SR_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                  # Reserve for spinning reserve provided by all BESS units in hour t [MWh]
    ...
    HydroBoost_variable_ht_real(JuMP_model, am, :hydro, "r_RU_G_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                  # Hydro reserve for regulation up provided by all hydro generators in hour t, when in generation mode [MWh]
    HydroBoost_variable_ht_real(JuMP_model, am, :hydro, "r_RD_G_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                  # Hydro reserve for regulation down provided by all hydro generators in hour t, when in generation mode [MWh]
    HydroBoost_variable_ht_real(JuMP_model, am, :hydro, "r_SR_G_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                  # Hydro reserve for spinning reserve provided by all hydro generators in hour t, when in generation mode [MWh]
    
    HydroBoost_variable_ht_real(JuMP_model, am, :hydro, "r_RU_GB_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                 # Hydro + BESS reserve for regulation up provided by all hydro generators in hour t [MWh]
    HydroBoost_variable_ht_real(JuMP_model, am, :hydro, "r_RD_GB_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                 # Hydro + BESS reserve for regulation down provided by all hydro generators in hour t [MWh]
    HydroBoost_variable_ht_real(JuMP_model, am, :hydro, "r_SR_GB_ht", ids_h, ids_t; bounded_lower=true, lower_bound=0.0)                                                                 # Hydro + BESS reserve for spinning reserve provided by all hydro generators in hour t [MWh]
    ...
    for h in ids_h
        for t in ids_t
            ...
            # Total discharge and charge power provided by BESS
            ...
            ### REMOVED HydroBoost_constraint_total_ES_soc_ht(JuMP_model, am, "HydroBoost_constraint_total_ES_soc_ht", ids_i_sto, h, t; const_name_flag)  
            
            # Total reg up, reg down, and spinning reserve provided by BESS units                                       
            HydroBoost_constraint_total_ES_reg_up_reserve_ht(JuMP_model, am, "HydroBoost_constraint_total_ES_reg_up_reserve_ht", ids_i_sto, h, t; const_name_flag)
            HydroBoost_constraint_total_ES_reg_dn_reserve_ht(JuMP_model, am, "HydroBoost_constraint_total_ES_reg_dn_reserve_ht", ids_i_sto, h, t; const_name_flag)
            HydroBoost_constraint_total_ES_spin_reserve_ht(JuMP_model, am, "HydroBoost_constraint_total_ES_spin_reserve_ht", ids_i_sto, h, t; const_name_flag)
    ...
    ### Hydro power system constraints: from (30) to (78) ###
    for h in ids_h
        for t in ids_t

            # Total generation of hydro generators
            ...
            HydroBoost_constraint_total_reg_up_reserve_jht(JuMP_model, am, "HydroBoost_constraint_total_reg_up_reserve_jht", ids_j_hydro, h, t; const_name_flag)
            HydroBoost_constraint_total_reg_dn_reserve_jht(JuMP_model, am, "HydroBoost_constraint_total_reg_dn_reserve_jht", ids_j_hydro, h, t; const_name_flag)
            HydroBoost_constraint_total_spin_reserve_jht(JuMP_model, am, "HydroBoost_constraint_total_spin_reserve_jht", ids_j_hydro, h, t; const_name_flag)
    ...
    ### Startup and shutdown daily limit of hydro generators and hydro pumps
    ...
    for d in 1:num_days
        ...
        for j in ids_j_pump
            HydroBoost_constraint_hydro_pumps_daily_caps_jd(JuMP_model, am, "HydroBoost_constraint_hydro_pumps_daily_caps_jd", j, d, ids_t; const_name_flag)
        end
    end

    ### RES generators constraints: from (79) to (81) ###
    for h in ids_h
        for t in ids_t
            for k in ids_k_ren
    ...
    ### Coupling constraints: from (82) to (89) ###
    for h in ids_h
        for t in ids_t
            HydroBoost_constraint_reg_up_sale_ht(JuMP_model, am, "HydroBoost_constraint_reg_up_sale_ht", h, t; const_name_flag)
            HydroBoost_constraint_reg_dn_sale_ht(JuMP_model, am, "HydroBoost_constraint_reg_dn_sale_ht", h, t; const_name_flag)
            HydroBoost_constraint_spin_sale_ht(JuMP_model, am, "HydroBoost_constraint_spin_sale_ht", h, t; const_name_flag)
        end
    end
    ...
    ###################################
    HydroBoost_objective_function(JuMP_model, am, ids_j_hydro, ids_j_pump, ids_i_sto, ids_h, ids_t, ids_l)
...
function HydroBoost_objective_function(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, ids_j_hydro, ids_j_pump, ids_i_sto, ids_h, ids_t, ids_l)
    ...
    for h in ids_h
        for t in ids_t
            JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["Regulation_up"], get_variable(am, :r_RU_ht, (h,t)))
            JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["Regulation_down"], get_variable(am, :r_RD_ht, (h,t)))
            JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["Spin"], get_variable(am, :r_SR_ht, (h,t)))
        end
    end
    ...
    for h in ids_h
        for t in ids_t
            JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["Regulation_up"], get_variable(am, :r_RU_G_ht, (h,t)))
            JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["Regulation_down"], get_variable(am, :r_RD_G_ht, (h,t)))
            JuMP.add_to_expression!(objective, am.ref[:nw][0][:repdays]["data"][h][t]["Spin"], get_variable(am, :r_SR_G_ht, (h,t)))
        end
    end
    ...
    for h in ids_h
        for t in ids_t
            ...
            JuMP.add_to_expression!(objective, reg_up_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RU_ht, (h,t)))
            JuMP.add_to_expression!(objective, - reg_down_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RD_ht, (h,t)))  
        end
    end
    ...
    for h in ids_h
        for t in ids_t
            ...
            JuMP.add_to_expression!(objective, reg_up_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RU_G_ht, (h,t)))
            JuMP.add_to_expression!(objective, - reg_down_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RD_G_ht, (h,t)))    
        end
    end
    ...
    # Operational costs: hydro generator start-up / shut-down
    for j in ids_j_hydro
        bus_idx  = parameter(am, 0, :gen_index, "bus_idx",      j)
        tech_idx = parameter(am, 0, :gen_index, "genco_tech_id", j)
        START_UP_COST_G = parameter(am, bus_idx, :gen_bus, tech_idx, "START_UP_COST")
        SHUT_DN_COST_G  = parameter(am, bus_idx, :gen_bus, tech_idx, "SHUT_DN_COST")

        for h in ids_h
            for t in ids_t
                JuMP.add_to_expression!(objective, -START_UP_COST_G, get_variable(am, :a_G_jht, (j,h,t)))
                JuMP.add_to_expression!(objective, -SHUT_DN_COST_G, get_variable(am, :z_G_jht, (j,h,t)))
            end
        end
    end

    # Operational costs: PUMP start-up / shut-down
    for j in ids_j_pump
        bus_idx  = parameter(am, 0, :gen_index, "bus_idx",      j)
        tech_idx = parameter(am, 0, :gen_index, "genco_tech_id", j)
        START_UP_COST_P = parameter(am, bus_idx, :gen_bus, tech_idx, "START_UP_COST")
        SHUT_DN_COST_P  = parameter(am, bus_idx, :gen_bus, tech_idx, "SHUT_DN_COST")

        for h in ids_h
            for t in ids_t
                JuMP.add_to_expression!(objective, -START_UP_COST_P, get_variable(am, :a_P_jht, (j,h,t)))
                JuMP.add_to_expression!(objective, -SHUT_DN_COST_P, get_variable(am, :z_P_jht, (j,h,t)))
            end
        end
    end
...
# Constraint (26)
function HydroBoost_constraint_total_ES_reg_up_reserve_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, ids_i_sto, h::Int, t::Int; const_name_flag::Bool=false)
    
    # parameter  
    
    # variable
    r_RU_ht = get_variable(am, :r_RU_ht, (h,t))
...
# Constraint (27)
function HydroBoost_constraint_total_ES_reg_dn_reserve_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, ids_i_sto, h::Int, t::Int; const_name_flag::Bool=false)
    
    # parameter  
    
    # variable
    r_RD_ht = get_variable(am, :r_RD_ht, (h,t))
...
# Constraint (28)
function HydroBoost_constraint_total_ES_spin_reserve_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, ids_i_sto, h::Int, t::Int; const_name_flag::Bool=false)
    
    # parameter  
    
    # variable
    r_SR_ht = get_variable(am, :r_SR_ht, (h,t))
...
# Constraint (58)
function HydroBoost_constraint_total_reg_up_reserve_jht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, ids_j_hydro, h::Int, t::Int; const_name_flag::Bool=false)

    # parameter

    # variable
    r_RU_G_ht = get_variable(am, :r_RU_G_ht, (h,t))
...
# Constraint (59)
function HydroBoost_constraint_total_reg_dn_reserve_jht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, ids_j_hydro, h::Int, t::Int; const_name_flag::Bool=false)
    
    # parameter  
    
    # variable
    r_RD_G_ht = get_variable(am, :r_RD_G_ht, (h,t))
...
# Constraint (60)
function HydroBoost_constraint_total_spin_reserve_jht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, ids_j_hydro, h::Int, t::Int; const_name_flag::Bool=false)
    
    # parameter  
    
    # variable
    r_SR_G_ht = get_variable(am, :r_SR_G_ht, (h,t))
...
# Constraints (68) --> Already included in the water balance equation - constraint (66)
...
# (t) Startup and shutdown daily limit of hydro generators and hydro pumps
...
# Constraint (78)
function HydroBoost_constraint_hydro_pumps_daily_caps_jd(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, j::Int, d::Int, ids_t; const_name_flag::Bool=false)
    ...
    for h in h_start:h_end
        for t in ids_t
            JuMP.add_to_expression!(sum_a, 1.0, get_variable(am, :a_P_jht, (j,h,t)))
        end
    end
...
# Constraint (81) is already included in section "Define decision variables".
...
# Constraint (82)
function HydroBoost_constraint_reg_up_sale_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, h::Int, t::Int; const_name_flag::Bool=false)

    # variable
    r_RU_GB_ht = get_variable(am, :r_RU_GB_ht, (h,t))
    r_RU_ht = get_variable(am, :r_RU_ht, (h,t))
    r_RU_G_ht = get_variable(am, :r_RU_G_ht, (h,t))
   
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        r_RU_GB_ht - r_RU_ht - r_RU_G_ht
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr == 0
        )    
...
# Constraint (83)
function HydroBoost_constraint_reg_dn_sale_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, h::Int, t::Int; const_name_flag::Bool=false)
    
    # variable
    r_RD_GB_ht = get_variable(am, :r_RD_GB_ht, (h,t))
    r_RD_ht = get_variable(am, :r_RD_ht, (h,t))
    r_RD_G_ht = get_variable(am, :r_RD_G_ht, (h,t))
    
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        r_RD_GB_ht - r_RD_ht - r_RD_G_ht
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr == 0
        )    
...
# Constraint (84)
function HydroBoost_constraint_spin_sale_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, h::Int, t::Int; const_name_flag::Bool=false)

    # variable
    r_SR_GB_ht = get_variable(am, :r_SR_GB_ht, (h,t))
    r_SR_ht = get_variable(am, :r_SR_ht, (h,t))
    r_SR_G_ht = get_variable(am, :r_SR_G_ht, (h,t))
    
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        r_SR_GB_ht - r_SR_ht - r_SR_G_ht
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr == 0
        )    
...
function HydroBoost_constraint_outflow_interconnection_limit_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, h::Int, t::Int; const_name_flag::Bool=false)
    ...
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        (p_B_DT_ht + p_G_Gr_ht + p_R_Gr_ht + r_RU_GB_ht + r_SR_GB_ht) - outflow_limits
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr <= 0
        )    
...
function HydroBoost_constraint_inflow_interconnection_limit_ht(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, const_name::String, h::Int, t::Int; const_name_flag::Bool=false)
    ...
    # constraint
    expr = JuMP.@expression(JuMP_model,  
        p_Gr_St_ht + r_RD_GB_ht - inflow_limits
        )   
    constraint = JuMP.@constraint(JuMP_model, 
        expr <= 0
        )    
...
function HydroBoost_report_result_plant_dispatch(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data)
    ...
    # Write Label 
    dispatch_label_list = ["day", "hour", "time", "LMP", "RT_LMP", "Reg Up Price", "Reg Dn Price", "Spin Price", "Delta RU", "Delta RD",
    "p_G_ht", "p_P_ht", "p_GB_ht", "p_G_Gr_ht", "r_RU_G_ht", "r_RD_G_ht", "r_SR_G_ht", "q_G_jht", "q_P_TOT_jht", "e_H_ht",
    "p_B_DT_ht", "p_B_CT_ht", "p_R_ht", "p_R_Gr_ht", "p_R_St_ht", "p_Gr_St_ht", "r_RU_ht", "r_RD_ht", "r_SR_ht",
    "r_RU_GB_ht", "r_RD_GB_ht", "r_SR_GB_ht"]
    ...
    for day_id in 1:num_days
        for hour_id in ids_h
            for time_id in ids_t
                ...
                ### REMOVED ### e_B_ht = daily_solutions[string(day_id)]["solution"]["storage"][idx_ht]["e_B_ht"]
                p_Gr_St_ht = daily_solutions[string(day_id)]["solution"]["storage"][idx_ht]["p_Gr_St_ht"]
                r_RU_ht = daily_solutions[string(day_id)]["solution"]["storage"][idx_ht]["r_RU_ht"]
                r_RD_ht = daily_solutions[string(day_id)]["solution"]["storage"][idx_ht]["r_RD_ht"]
                r_SR_ht = daily_solutions[string(day_id)]["solution"]["storage"][idx_ht]["r_SR_ht"]
                ...
                r_RU_G_ht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_ht]["r_RU_G_ht"]
                r_RD_G_ht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_ht]["r_RD_G_ht"]
                r_SR_G_ht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_ht]["r_SR_G_ht"]

                # Hydro + BESS
                r_RU_GB_ht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_ht]["r_RU_GB_ht"]
                r_RD_GB_ht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_ht]["r_RD_GB_ht"]
                r_SR_GB_ht = daily_solutions[string(day_id)]["solution"]["hydro"][idx_ht]["r_SR_GB_ht"]
                ...
                dispatch_output_list[row_id, :] = [day_id, hour_id, time_id, LMP, RT_LMP, Reg_up_price, Reg_dn_price, Spin_price, Delta_RU, Delta_RD,
                p_G_ht, p_P_ht, p_GB_ht, p_G_Gr_ht, r_RU_G_ht, r_RD_G_ht, r_SR_G_ht, q_G_jht, q_P_TOT_jht, e_H_ht,
                p_B_DT_ht, p_B_CT_ht, p_R_ht, p_R_Gr_ht, p_R_St_ht, p_Gr_St_ht, r_RU_ht, r_RD_ht, r_SR_ht,
                r_RU_GB_ht, r_RD_GB_ht, r_SR_GB_ht]
            end
...
function HydroBoost_report_result_objective(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data)
    ...
    # Pump unit set (same logic as where ids_j_pump is defined for the objective) --- NEW
    ids_j_pump = [j for j in get_index(am, :gen_index, 0)
                  if parameter(am, 0, :gen_index, "UNIT_CATEGORY", j) == "HYDRO" &&
                  begin
                      bus_idx  = parameter(am, 0, :gen_index, "bus_idx",      j)
                      tech_idx = parameter(am, 0, :gen_index, "genco_tech_id", j)
    ...
    rz_penalty = get(am.setting["Simulation Setting"], "hyropower_rough_zone_operation_penalty_value", 0.0)
    ...
    for day_id in repday_ids
        ...
        for h in ids_h, t in ids_t
            ...
            energy_arbitrage = LMP * (p_G_Gr_ht + p_B_DT_ht + p_R_Gr_ht - 1.00001 * p_Gr_St_ht)
            energy_import_cost = LMP * p_Gr_St_ht

            # AS/reg-adjust terms aligned with the objective: aggregate ht variables
            r_RU_ht   = daily_solutions[day_key]["solution"]["storage"][idx_ht]["r_RU_ht"]
            r_RD_ht   = daily_solutions[day_key]["solution"]["storage"][idx_ht]["r_RD_ht"]
            r_SR_ht   = daily_solutions[day_key]["solution"]["storage"][idx_ht]["r_SR_ht"]
            r_RU_G_ht = daily_solutions[day_key]["solution"]["hydro"][idx_ht]["r_RU_G_ht"]
            r_RD_G_ht = daily_solutions[day_key]["solution"]["hydro"][idx_ht]["r_RD_G_ht"]
            r_SR_G_ht = daily_solutions[day_key]["solution"]["hydro"][idx_ht]["r_SR_G_ht"]

            # AS: BESS
            as_bess_reg_up   = Pru * r_RU_ht
            as_bess_reg_down = Prd * r_RD_ht
            as_bess_spin     = Pspin * r_SR_ht
            as_bess          = as_bess_reg_up + as_bess_reg_down + as_bess_spin
            reg_adj_bess     = reg_up_signal * RT_LMP * r_RU_ht - reg_down_signal * RT_LMP * r_RD_ht

            # AS: Hydro
            as_hydro_reg_up   = Pru * r_RU_G_ht
            as_hydro_reg_down = Prd * r_RD_G_ht
            as_hydro_spin     = Pspin * r_SR_G_ht
            as_hydro          = as_hydro_reg_up + as_hydro_reg_down + as_hydro_spin
            reg_adj_hydro     = reg_up_signal * RT_LMP * r_RU_G_ht - reg_down_signal * RT_LMP * r_RD_G_ht
            ...
            # Startup/Shutdown cost: pumps (P-mode)
            for j in ids_j_pump
                key_jht = string("(", j, ", ", h, ", ", t, ")")
                sol_h = daily_solutions[day_key]["solution"]["hydro"][key_jht]

                # Safe getters for pump start-up/shut-down variables
                a_P = get_or0(sol_h, "a_P_jht")
                z_P = get_or0(sol_h, "z_P_jht")
                ...
                startup_shutdown_cost += -(c_su * a_P + c_sd * z_P)
            end
...
function generate_networkdata_HydroBoost(ALEAF_setting::Dict{String,<:Any}, project_id, case_id; year_id::Int64=1)
    ...
    forecasting_method = ALEAF_setting["Simulation Setting"]["Market_price_forecasting_method"]  # Perfect_foresight, Mean_persistence, Random_forest_with_regressors, Random_forest_no_regressors, Additive_model_with_regressors, Additive_model_no_regressors, Autoregressive_with_regressors, Autoregressive_no_regressors, Manual_Forecast

    if forecasting_method in ["Perfect_foresight", "Mean_persistence", "Random_forest_with_regressors", "Random_forest_no_regressors", "Additive_model_with_regressors", "Additive_model_no_regressors", "Autoregressive_with_regressors", "Autoregressive_no_regressors", "Manual_Forecast"]
