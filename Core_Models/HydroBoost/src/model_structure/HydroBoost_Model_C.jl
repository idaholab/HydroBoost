...
function build_HydroBoost_optimization_model_instance!(am::Abstract_ALEAF_Model, day_id, prior_day_solution)
    ...
    ### Hydro power system constraints: from (31) to (79) ###
    for h in ids_h
        for t in ids_t
    ...
    ### RES generators constraints: from (80) to (82) ###
    for h in ids_h
        for t in ids_t
            for k in ids_k_ren
    ...
    ### Coupling constraints: from (83) to (90) ###
    for h in ids_h
        for t in ids_t
...
function HydroBoost_objective_function(JuMP_model::JuMP.AbstractModel, am::Abstract_ALEAF_Model, ids_j_hydro, ids_j_pump, ids_i_sto, ids_h, ids_t, ids_l)
    ...
    for h in ids_h
        for t in ids_t
            for i in ids_i_sto
                ...
                JuMP.add_to_expression!(objective, reg_up_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RU_iht, (i,h,t)))
                JuMP.add_to_expression!(objective, - reg_down_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RD_iht, (i,h,t)))                
            end
        end
    end
    ...
    for h in ids_h
        for t in ids_t
            for j in ids_j_hydro
                ...
                JuMP.add_to_expression!(objective, reg_up_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RU_G_jht, (j,h,t)))
                JuMP.add_to_expression!(objective, - reg_down_signal * am.ref[:nw][0][:repdays]["data"][h][t]["RT_LMP"], get_variable(am, :r_RD_G_jht, (j,h,t)))                
            end
        end
    end
    ...
    for j in ids_j_hydro
        ...
        for h in ids_h
            for t in ids_t
                JuMP.add_to_expression!(objective, -START_UP_COST_G,
                                        get_variable(am, :a_G_jht, (j,h,t)))
                JuMP.add_to_expression!(objective, -SHUT_DN_COST_G,
                                        get_variable(am, :z_G_jht, (j,h,t)))
            end
        end
    end
    ...
    for j in ids_j_pump
        ...
        for h in ids_h
            for t in ids_t
                JuMP.add_to_expression!(objective, -START_UP_COST_P,
                                        get_variable(am, :a_P_jht, (j,h,t)))
                JuMP.add_to_expression!(objective, -SHUT_DN_COST_P,
                                        get_variable(am, :z_P_jht, (j,h,t)))
            end
        end
    end
...
# Constraints (33) and (34) are already included in section "Define decision variables".
...
# Constraints (37) and (38) already included in section "Define decision variables".

# Constraint (39): group-level operating mode exclusivity (either pumps OR generators)
...
# Constraint (59), (60), and (61) are already included in the objective function formulation.
...
# Constraints (69) --> Already included in the water balance equation - constraint (66)
...
# Constraints (75) and (76) are already included in section "Define decision variables".
...
# Constraint (82) is already included in section "Define decision variables".

### Coupling constraints ###

# (v) Ancillary services offered to the market:

# Constraints (83), (84), and (85) are already included in the objective function formulation.
...
function HydroBoost_report_result_plant_dispatch(am::Abstract_ALEAF_Model, ALEAF_setting, daily_solutions, project_id, network_data)
    ...
    # Write Label 
    dispatch_label_list = ["day", "hour", "time", "LMP", "RT_LMP", "Reg Up Price", "Reg Dn Price", "Spin Price", "Delta RU", "Delta RD", "p_G_ht", "p_P_ht", "p_GB_ht", "p_G_Gr_ht", "q_G_jht", "q_P_TOT_jht", "e_H_ht", "p_B_DT_ht", "p_B_CT_ht", "e_B_ht",  "p_R_ht", "p_R_Gr_ht", "p_R_St_ht", "p_Gr_St_ht"]
