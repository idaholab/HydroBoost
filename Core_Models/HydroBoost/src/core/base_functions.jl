...
function parameter(am::Abstract_ALEAF_Model, nw::Int, key1::Symbol, idx::Int)
    # Some refs are optional per-plant collections (e.g., a plant may have no storage units).
    # For these, return an empty vector instead of throwing a KeyError.
    if key1 == :sto_index || key1 == :hydro_index
        return get(am.ref[:nw][nw][key1], idx, Int[])
    elseif key1 == :renw_index
        return get(am.ref[:nw][nw][key1], idx, String[])
    end
